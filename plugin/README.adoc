:coherence-gradle-plugin-version: 1.0.0-SNAPSHOT
...
Set the boolean `debug` property to `true` in order to provide additional logging output in regards the instrumented classes.
...
plugins {
  id 'java'
  id 'com.oracle.coherence.gradle' version '{coherence-gradle-plugin-version}'
...
changes reapidly reflected in the execution of the sample module.
